...
function AddTrek() {
  ...
  const updateDeparture = (index, field, value) => {
    setForm((current) => ({
      ...current,
  ...
  const updateGearTitle = (
    ...
    setForm((current) => ({
      ...current,
      gearSections: current.gearSections.map(
        (section, i) =>
          i === sectionIndex
            ? {
                ...section,
  ...
  const addGearItem = (sectionIndex) => {
    setForm((current) => ({
      ...current,
      gearSections: current.gearSections.map(
        (section, i) =>
  ...
  const removeGearSection = (index) => {
    setForm((current) => ({
      ...current,
      gearSections: current.gearSections.filter(
        (_, i) => i !== index
      ),
    }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (
        ...
        throw new Error(
          result.message ||
            `Failed to save trek. Server returned ${response.status}.`
        );
      }
       ...
       * before returning to the admin dashboard.
       */
      ...
      if (
        ...
        setError(
          error.message ||
            "Something went wrong while saving the trek."
        );
      }
    }
  };
    ...
    <div className="add-trek-page">
      ...
      <div className="add-trek-header">
        ...
        <div>
          ...
          <p>
            Create a complete trekking package
            with content, itinerary, pricing,
            media and SEO.
          </p>
        </div>
